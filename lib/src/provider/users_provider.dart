...
class UsersProvider {
  ...
  Future<Stream> createWithImage(User user, File image) async {
    try {
      ...
      final response = await request.send(); // Enviará la petición
  ...
  Future<ResponseApi> logout(String idUser) async {
    try {
      ...
      String bodyParams =
          json.encode({'id': idUser}); //recibe todos los parametros de user
      Map<String, String> headers = {'Content-type': 'application/json'};
      final res = await http.post(url, headers: headers, body: bodyParams);
      final data = json.decode(
          res.body); //data almacena la respuesta (message,error,success)
  ...
  Future<ResponseApi> login(String email, String password) async {
    try {
      ...
      String bodyParams = json.encode(
          {'email': email, 'password': password}); //enviamos un mapa de valores
      Map<String, String> headers = {'Content-type': 'application/json'};
      final res = await http.post(url, headers: headers, body: bodyParams);
      final data = json.decode(
          res.body); //data almacena la respuesta (message,error,success)
